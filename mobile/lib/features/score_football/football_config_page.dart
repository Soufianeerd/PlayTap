...
import '../../domain/rulesets/football_rulesets.dart';
import '../../domain/rulesets/team_match_format.dart';
...
class FootballConfigPage extends ConsumerStatefulWidget {
  const FootballConfigPage({super.key});

  @override
  ConsumerState<FootballConfigPage> createState() => _FootballConfigPageState();
}

class _FootballConfigPageState extends ConsumerState<FootballConfigPage> {
  TeamMatchFormat _format = TeamMatchFormat.league;
  ...
  Future<void> _onStart() async {
    ...
    final matchRule = resolveDefaultFootballRuleset(
      DateTime.now().toUtc(),
      _format,
    );
    final sessionId = await startTeamMatchSession(
      ...
      teamA: teamA,
      teamB: teamB,
      presetRef: footballPresetRef,
    );
    if (mounted) context.pushReplacement('/score/football/session/$sessionId');
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(l10n.footballConfigTitle)),
      body: Column(
        children: [
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    l10n.teamMatchFormatLabel,
                    style: PlayTapTypography.label.copyWith(
                      color: colors.muted,
                    ),
                  ),
                  const SizedBox(height: PlayTapSpacing.sm),
                  PillSelector<TeamMatchFormat>(
                    options: TeamMatchFormat.values,
                    labelBuilder: (f) => switch (f) {
                      TeamMatchFormat.league => l10n.teamMatchFormatLeague,
                      TeamMatchFormat.knockout => l10n.teamMatchFormatKnockout,
                    },
                    value: _format,

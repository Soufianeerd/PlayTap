...
import '../../domain/rulesets/futsal_rulesets.dart';
...
class FutsalConfigPage extends ConsumerStatefulWidget {
  const FutsalConfigPage({super.key});

  @override
  ConsumerState<FutsalConfigPage> createState() => _FutsalConfigPageState();
}

class _FutsalConfigPageState extends ConsumerState<FutsalConfigPage> {
  ...
  Future<void> _onStart() async {
    ...
    final matchRule = resolveDefaultFutsalRuleset(
      DateTime.now().toUtc(),
      _format,
    );
    final sessionId = await startTeamMatchSession(
      ...
      teamA: teamA,
      teamB: teamB,
      presetRef: futsalPresetRef,
    );
    if (mounted) context.pushReplacement('/score/futsal/session/$sessionId');
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(l10n.futsalConfigTitle)),
      body: Column(
        children: [
          Expanded(
